#!/usr/bin/env python3
"""Collect standalone Redis metadata, validate results, and render a report."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual Redis version is empty")
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# Redis {result['version']} 独立性能测试报告",
        "",
    ...
    for label, field in (
        ...
        lines.append(f"| {label} | {markdown_cell(result['build_info'].get(field))} |")
...
def finalize(
    ...
    benchmark = load_json(output_dir / "benchmark_redis.json")
    error = ""
    metrics: dict[str, dict[str, Any]] = {}
    if command_status == "passed":
        try:
            if not (output_dir / "redis_benchmark_raw.log").is_file():
                raise RuntimeError("redis_benchmark_raw.log is missing")
