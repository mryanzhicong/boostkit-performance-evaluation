...
- `case.yaml`：使用 `execution.stages` 显式声明 build、start、test、stop 各自的 Shell 脚本和函数入口，使用顶级 `outputs` 声明软件结果的逻辑名称、路径、生成阶段、格式和必要性，并让 `metrics` 通过逻辑输出名声明指标提取规则；不得声明架构、Runner 标签或 Framework 统一生成的 `build_info.json`。
- `<software>_test.sh`：已有测试用例的阶段函数实现，通过环境变量接收版本、架构和输出目录。四个阶段键固定为 build、start、test、stop，但对应函数应使用 `build_<software>`、`start_<software>_service`、`run_<software>_benchmarks`、`stop_<software>_service` 一类易读名称，并在 `case.yaml` 中显式映射。build 必须从实际产物读取版本，并向 `PERF_ACTUAL_VERSION_FILE` 写入一个非空单行值；脚本被加载时不得自动执行阶段，也不再接收阶段名进行分发。
...
Framework 会校验软件报告的实际版本必须等于请求版本并生成 `build_info.json`；CPU、系统、内核、Python、GCC、glibc 和 NUMA 等静态信息只统一采集一次，内存和 CPU governor 等动态状态才在测试前后采集，软件目录不得重复实现版本环境信息脚本。
