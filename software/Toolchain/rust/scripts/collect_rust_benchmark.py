#!/usr/bin/env python3
"""Collect the ripgrep benchmark rounds from the raw log into results.json."""
...
EXPECTED_WORKLOADS = {
    "compile": ("dev", "release", "opt-level=s"),
    "search": (64, 512),
}
MIN_ROUNDS = 2
...
def metric_prefix(scenario: str, workload: str) -> str:
    if scenario == "compile":
        return f"compile ripgrep --profile={workload}"
    return f"ripgrep search --corpus={workload}MiB"


def main() -> int:
    if len(sys.argv) != 3:
        print(
            "usage: collect_rust_benchmark.py RAW_LOG OUTPUT",
            file=sys.stderr,
        )
        return 1
    ...
    if missing:
        raise ValueError("ripgrep benchmark is missing results: " + ", ".join(missing))
    ...
    for (scenario, workload), entry in sorted(rounds.items()):
        ...
        for field, unit in FIELDS:
            ...
            if source_name in results:
                raise ValueError(f"duplicate ripgrep benchmark metric: {source_name}")
            results[source_name] = {
    ...
    print(f"[rust] recorded {len(results)} ripgrep benchmark metrics")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"[rust] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
