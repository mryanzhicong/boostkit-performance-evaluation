...
The raw output is produced by ``gcc_test.sh``: one line per officially
compiled translation unit, ``<source-file-name> <elapsed-nanoseconds>``.
...
All timings are wall-clock durations in nanoseconds (from ``date +%s%N``)
...
# One line: "<source-file> <elapsed-ns>".
LINE_RE = re.compile(r"^(?P<name>\S+\.c)\s+(?P<ns>\d+)$")
...
def parse_line(line: str, line_number: int, results: dict[str, Any]) -> None:
    ...
    results[name] = {
        ...
        "raw_value": elapsed_ns,
        "raw_unit": "ns",
        "value": elapsed_ns / 1.0e9,
        "unit": "s",
...
def main() -> int:
    ...
    try:
        ...
        return 1
    try:
        skipped = int(os.environ.get("GCC_CORPUS_SKIPPED", "0"))
    except ValueError:
        fail("GCC_CORPUS_SKIPPED is not an integer")
        return 1
    ...
    try:
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            if not stripped:
                continue
            parse_line(stripped, line_number, results)
    ...
    if not results:
        ...
        return 1
    if compiled and len(results) != compiled:
        fail(
            f"metric count {len(results)} differs from compiled count {compiled}"
        )
        return 1
    ...
    normalized = {
        ...
        "parameters": {
            ...
            "opt_level": opt_level,
            "timing_backend": "date +%s%N wall clock",
            "compiled": compiled,
            "skipped": skipped,
        },
        "metric_contract": {
