#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for glibc."""
...
def record_build_info(
    ...
    source_url: str,
    benchset: str,
    use_clock_gettime: str,
) -> None:
    actual_version = actual_version_file.read_text(encoding="utf-8").strip()
    if not actual_version:
        raise RuntimeError("actual glibc version is empty")
    atomic_write_json(
        output,
        {
            ...
            "source_tarball": f"glibc-{requested_version}.tar.xz",
            "build_command": (
                "configure --prefix=<install> --disable-werror && "
                "make -j$(nproc)"
            ),
            "benchset": benchset.split(),
...
def extract_metrics(
    ...
    if benchmark.get("software") != "glibc":
        raise RuntimeError("benchmark_glibc.json has an invalid software identity")
    if (
        benchmark.get("version") != version
        or benchmark.get("architecture") != architecture
    ):
        raise RuntimeError("benchmark_glibc.json identity differs from this run")
    results = benchmark.get("results")
    if not isinstance(results, dict) or not results:
        raise RuntimeError("benchmark_glibc.json is missing results")
    metrics: dict[str, Any] = {}
    for result_key, result in results.items():
        if not isinstance(result, dict):
            raise TypeError(f"glibc benchmark {result_key} must be an object")
        metric_name = result.get("source_name")
        if not isinstance(metric_name, str) or not metric_name:
            raise RuntimeError(f"glibc benchmark {result_key} has no source_name")
        if metric_name != result_key:
            raise RuntimeError(
                f"glibc benchmark key {result_key} differs from source_name {metric_name}"
            )
        ...
        if result.get("unit") != "ns":
            raise RuntimeError(f"metric {metric_name} is not expressed in ns")
    ...
    if not metrics:
        raise RuntimeError("benchmark_glibc.json contains no metrics")
    return metrics
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# glibc {result['version']} 独立性能测试报告",
        "",
    ...
    lines.extend(
        [
            "",
            "## 性能指标（官方 glibc benchtests 逐字 JSON 路径，clock_gettime 纳秒）",
            "",
            "| 指标 | 数值 | 单位 | 优化方向 |",
            "|---|---:|---|---|",
        ]
...
def finalize(
    ...
    benchmark = load_json(output_dir / "benchmark_glibc.json")
    error = ""
    metrics: dict[str, Any] = {}
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_glibc.json is missing or invalid")
...
def parse_args() -> argparse.Namespace:
    ...
    build.add_argument(
        "--source-url",
        required=True,
        help="GNU mirror hosting the glibc release tarballs",
    )
    build.add_argument(
        "--benchset",
        required=True,
        help="benchtests BENCHSET selection used by make bench",
    )
    build.add_argument(
        "--use-clock-gettime",
        required=True,
        help="USE_CLOCK_GETTIME value forcing clock_gettime (ns) timing",
    )
