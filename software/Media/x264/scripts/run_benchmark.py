#!/usr/bin/env python3
"""Run x264's official H.264 encode benchmarks and normalize fps metrics.

x264 is a command-line H.264 encoder; its canonical throughput signal is the
``encoded N frames, X fps`` line printed to stderr on every run. This driver
reproduces the two official benchmark families shipped by the x264 test wrapper:
...
* **micro scaling** — encode at three resolutions (resolution scaling) and at
  several ``--threads`` counts (thread scaling) at the ``medium`` preset.

Every metric name is built verbatim from the official ``--preset`` value,
``--input-res`` resolution or ``--threads`` count plus the official ``fps``
field, so each name is directly traceable to the official x264 CLI.
"""
...
# "auto" omits --threads so x264 selects every available core; the remaining
# values pass an explicit thread count.
...
def fail(message: str) -> None:
    print(f"[x264-benchmark] ERROR: {message}", file=sys.stderr)
...
def run_once(
    ...
    command = build_command(cli_bin, yuv_file, width, height, frames, preset, threads)
    ...
    if completed.returncode:
        raise RuntimeError(
            f"x264 exited with code {completed.returncode}: {' '.join(command)}"
        )
...
def parse_stats(text: str) -> dict[str, float | int]:
    ...
    if not match:
        raise RuntimeError("x264 output has no 'encoded N frames, X fps' summary")
    return {
        "fps": float(match.group(2) or 0.0),
        "bitrate_kbps": float(match.group(3) or 0.0),
        "psnr_db": float(match.group(4) or 0.0),
...
def encode_preset_sweep(
    ...
    for preset in PRESETS:
        ...
        for _ in range(iterations):
            text = run_once(cli_bin, yuv_file, width, height, frames, preset)
        ...
        results[name] = {
            ...
            "bitrate_kbps": round(float(stats.get("bitrate_kbps", 0.0)), 4),
            "psnr_db": round(float(stats.get("psnr_db", 0.0)), 4),
            "encode_time_s": round(frames / fps, 6) if fps > 0 else 0.0,
        }
    return results
...
def micro_resolution_scaling(
    ...
    results: dict[str, dict[str, Any]] = {}
    tmpdir = tempfile.mkdtemp(prefix="x264_res_")
    try:
...
def micro_thread_scaling(
    ...
    tmpdir = tempfile.mkdtemp(prefix="x264_thread_")
    try:
        ...
        for threads in THREAD_COUNTS:
            raw_lines.append(f"### threads {threads} (1280x720, medium, 30 frames)")
            fps_values: list[float] = []
            for _ in range(iterations):
...
def main() -> int:
    if len(sys.argv) < 8:
        print(
            "usage: run_benchmark.py X264_BIN YUV_FILE WIDTH HEIGHT FRAMES "
    ...
    if not os.path.isfile(cli_bin) or not os.access(cli_bin, os.X_OK):
        fail(f"official x264 binary is unavailable: {cli_bin}")
        return 1
    ...
    try:
        ...
        return 1
    version_string = os.environ.get("X264_VERSION_STRING", version)

    raw_lines: list[str] = []
    try:
        raw_lines.append("# x264 official encode benchmark (preset sweep)")
        encode_results = encode_preset_sweep(
            cli_bin, yuv_file, width, height, frames, iterations, raw_lines
        )
        raw_lines.append("# x264 micro benchmark (resolution scaling)")
        resolution_results = micro_resolution_scaling(cli_bin, iterations, raw_lines)
        raw_lines.append("# x264 micro benchmark (thread scaling)")
    ...
    normalized = {
        "benchmark": "x264_official_encode_fps",
        "software": "x264",
        "version": version,
        "architecture": architecture,
        "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "parameters": {
            "command": ["x264", "--preset", "<preset>", "--fps", "25",
            ...
            "metric_source": "x264 CLI 'encoded N frames, X fps' summary",
        },
        "metric_contract": {
            "scope": (
                "verbatim x264 --preset / --input-res / --threads fps from "
        ...
        "runtime_context": {
            "x264_version_string": version_string,
        },
        "results": results,
    }
    ...
    print(f"[x264-benchmark] normalized {len(results)} fps metrics")
    return 0
