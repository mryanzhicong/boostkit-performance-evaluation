#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for x264."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("实际 x264 版本为空")
...
def extract_metrics(
    ...
    if benchmark.get("software") != "x264":
        raise RuntimeError("benchmark_x264.json 的 software 标识无效")
    if benchmark.get("version") != version or benchmark.get("architecture") != architecture:
        raise RuntimeError("benchmark_x264.json 的标识与本次运行不一致")
    results = benchmark.get("results")
    if not isinstance(results, dict):
        raise RuntimeError("benchmark_x264.json 缺少 results")
    metrics: dict[str, Any] = {}
    for result_key, result in results.items():
        if not isinstance(result, dict):
            raise RuntimeError(f"x264 结果 {result_key} 必须是对象")
        if result_key in metrics:
            raise RuntimeError(f"重复的 x264 指标：{result_key}")
    ...
    if not metrics:
        raise RuntimeError("benchmark_x264.json 不包含任何指标")
    return metrics
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# x264 {result['version']} 独立性能测试报告",
        "",
...
def finalize(
    ...
    benchmark = load_json(output_dir / "benchmark_x264.json")
    error = ""
    metrics: dict[str, Any] = {}
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_x264.json 缺失或无效")
