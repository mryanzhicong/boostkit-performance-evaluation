#!/usr/bin/env python3
"""Collect the data-plane benchmark run reports into results.json."""
...
# (mysql thread-ladder style).  The run-file slugs match dpdk_test.sh.
WORKLOADS = ("ring", "hash", "lpm")
...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "usage: collect_dpdk_benchmark.py RUNS_DIR OUTPUT",
            file=sys.stderr,
        )
        return 1
    ...
    for workload in WORKLOADS:
        for threads in THREADS_LADDER:
            ...
            for field, json_field, unit, direction in FIELDS:
                source_name = f"dpdk {workload} --threads={threads}: {field}"
        ...
        json.dumps(
            {
                ...
                "parameters": {
                    "build": "official source tarball, meson defaults "
                    "(max_numa_nodes=1)",
                    "eal": "--no-huge --in-memory -l 0 -m 1024",
                    "clock": "CLOCK_MONOTONIC (unit=ns)",
                    "warmup_seconds": int(
                        os.environ.get("DPDK_WARMUP_SECONDS", "5")
                    ),
                    "duration_seconds": int(
                        os.environ.get("DPDK_DURATION_SECONDS", "30")
                    ),
                    "threads_ladder": list(THREADS_LADDER),
                    "scenarios": {
                        "ring": "rte_ring SP/SC burst 32 enqueue+dequeue, "
                        "one dedicated ring pair per worker "
                        "(1 op = element enq+deq)",
                        "hash": "rte_hash lookups, 256k-entry flow table, "
                        "16-byte keys, 7/8 hits",
                        "lpm": "rte_lpm IPv4 lookups, 64k /24 routes, "
                        "7/8 hits",
                    },
                },
                "results": results,
    ...
    print(f"[dpdk] recorded {len(results)} data-plane benchmark metrics")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"[dpdk] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
