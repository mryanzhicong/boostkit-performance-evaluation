#!/usr/bin/env python3
"""Run the k6 static and servlet scenarios against Tomcat and normalize results."""
...
# Static-file scenarios served by the official ROOT webapp, the servlet
# scenario served by the official examples webapp, and the concurrency
# ladder (mysql thread-ladder style).  Each scenario runs once per VU level
# with HTTP keepalive (k6 keeps a per-VU cookie jar, so the servlet's
# session handling follows the official per-VU behaviour).
SCENARIOS = (
    ("file-1k.bin", "static 1KiB"),
    ("file-100k.bin", "static 100KiB"),
    ("examples/servlets/servlet/HelloWorldExample", "servlet hello"),
)
...
def run_scenario(
    ...
    summary = script.parent / f"benchmark_{label.replace(' ', '_')}_vus{vus}_summary.json"
    command = [
        str(k6),
        "run",
        "--vus",
    ...
    raw.write(f"[tomcat-k6] scenario: {label} --vus={vus}\n")
    raw.write("[tomcat-k6] command: " + " ".join(command) + "\n")
    completed = subprocess.run(
        command,
        env={**os.environ, "TOMCAT_TARGET": url},
        text=True,
...
def main() -> int:
    ...
    if not args.k6.is_file() or not (args.k6.stat().st_mode & 0o111):
        print("[tomcat-k6] ERROR: k6 executable is unavailable", file=sys.stderr)
        return 1
    duration = duration_seconds()
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.raw_output.parent.mkdir(parents=True, exist_ok=True)
    script = args.output.parent / "tomcat_k6_request.js"
    ...
    try:
        ...
        with args.raw_output.open("w", encoding="utf-8") as raw:
            for path, scenario in SCENARIOS:
                url = f"{args.url_base.rstrip('/')}/{path}"
                for vus in VUS_LADDER:
                    summary = run_scenario(
                        args.k6, script, scenario, url, vus, duration, raw
                    )
                    extracted = parse_summary(summary, f"{scenario} --vus={vus}")
                    for field, (result, unit, direction) in extracted.items():
                        source_name = f"k6 {scenario} --vus={vus}: {field}"
        ...
        if not results:
            raise RuntimeError("k6 produced no Tomcat metrics")
        ...
        print(f"[tomcat-k6] ERROR: {exc}", file=sys.stderr)
        return 1
    ...
    print(f"[tomcat-k6] normalized {len(results)} static and servlet metrics")
    return 0
