#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for kubernetes."""
...
def cpu_model() -> str:
    for line in Path("/proc/cpuinfo").read_text(
        ...
        field, separator, value = line.partition(":")
        if separator and field.strip() in ("model name", "Processor"):
            if value.strip():
                return value.strip()
    return "unknown"
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual kubernetes version is empty")
...
def extract_metrics(
    aggregate: dict[str, Any], version: str, architecture: str
) -> dict[str, Any]:
    if aggregate.get("software") != SOFTWARE_NAME:
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# Kubernetes {result['version']} 独立性能测试报告",
        "",
...
def finalize(
    ...
    aggregate = load_json(output_dir / "results.json")
    error = ""
    metrics: dict[str, Any] = {}
    if command_status == "passed":
        try:
            if not aggregate:
                raise RuntimeError("results.json is missing or invalid")
            metrics = extract_metrics(aggregate, version, architecture)
