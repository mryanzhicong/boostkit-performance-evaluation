 ...
 public:
  ...
  void run() {
    ...
    while (ops_->outstandingOps() < max_outstanding_ops_ && !exiting_) {
      ...
      auto cb = std::make_unique<LoadCallback<AsyncClient>>(
          this, ops_.get(), op, stats_);
...
class LoadCallback : public RequestCallbackWithValidator {
 public:
  LoadCallback(
      ...
      OP_TYPE op,
      QPSStats* stats)
      : runner_(runner), ops_(ops), op_(op), stats_(stats) {}
  ...
  void requestSent() override {
    if (isOneway_) {
      ...
      // Harness addition: timestamp the wire time of request/response
      // calls so replyReceived() can offer a round-trip sample.
      sentAt_ = std::chrono::steady_clock::now();
    }
  }
  void replyReceived(ClientReceiveState&& rstate) override {
    ...
    if (sentAt_) {
      stats_->latency().offer(static_cast<uint64_t>(
          std::chrono::duration_cast<std::chrono::nanoseconds>(
              std::chrono::steady_clock::now() - *sentAt_)
              .count()));
    }
 ...
 private:
  ...
  std::optional<std::chrono::steady_clock::time_point> sentAt_;
};
