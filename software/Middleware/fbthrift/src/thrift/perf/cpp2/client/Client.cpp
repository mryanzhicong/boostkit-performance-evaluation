...
DEFINE_string(transport, "header", "Transport to use: header, rocket");
...
// Harness addition: warm-up window before round-trip latency sampling
// begins (per-second QPS stats are printed from the start; the
// collector drops the first warmup_sec lines).
DEFINE_int32(warmup_sec, 0, "Seconds before latency sampling begins");
...
int main(int argc, char** argv) {
  ...
  bool measuring = FLAGS_warmup_sec <= 0; // harness addition
  stats.latency().setMeasuring(measuring);
  ...
  while (true) {
    ...
    // Harness addition: start latency sampling once warm-up elapsed.
    if (!measuring && elapsedTimeSec >= FLAGS_warmup_sec) {
      measuring = true;
      stats.latency().setMeasuring(true);
    }
  ...
  // Harness addition: one-shot round-trip latency summary.
