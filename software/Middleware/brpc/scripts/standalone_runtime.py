#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for brpc."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual brpc version is empty")
    atomic_write_json(
        output,
        {
...
def extract_metrics(
    ...
    if benchmark.get("software") != "brpc":
        raise RuntimeError("benchmark_brpc.json has an invalid software identity")
    if (
        benchmark.get("version") != version
        or benchmark.get("architecture") != architecture
    ):
        raise RuntimeError("benchmark_brpc.json identity differs from this run")
    results = benchmark.get("results")
    if not isinstance(results, dict) or not results:
        raise RuntimeError("benchmark_brpc.json is missing results")
    metrics: dict[str, Any] = {}
    for result_key, result in results.items():
        if not isinstance(result, dict):
            raise TypeError(f"brpc metric {result_key} must be an object")
        metric_name = result.get("source_name")
        if not isinstance(metric_name, str) or not metric_name:
            raise RuntimeError(f"brpc metric {result_key} has no source_name")
        if metric_name != result_key:
            raise RuntimeError(
                f"brpc metric key {result_key} differs from source_name {metric_name}"
            )
...
def direction_label(direction: str) -> str:
    return "越大越好" if direction == "higher_is_better" else "越小越好"


def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# brpc {result['version']} 独立性能测试报告",
        "",
...
def finalize(
    ...
    benchmark = load_json(output_dir / "benchmark_brpc.json")
    error = ""
    metrics: dict[str, Any] = {}
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_brpc.json is missing or invalid")
