#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for folly."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual folly version is empty")
    atomic_write_json(
        output,
        {
...
def extract_metrics(
    ...
    if benchmark.get("software") != "folly":
        raise RuntimeError("benchmark_folly.json has an invalid software identity")
    if (
        benchmark.get("version") != version
        or benchmark.get("architecture") != architecture
    ):
        raise RuntimeError("benchmark_folly.json identity differs from this run")
    results = benchmark.get("results")
    if not isinstance(results, dict) or not results:
        raise RuntimeError("benchmark_folly.json is missing results")
    metrics: dict[str, Any] = {}
    for result_key, result in results.items():
        if not isinstance(result, dict):
            raise TypeError(f"folly scenario {result_key} must be an object")
        metric_name = result.get("source_name")
        if not isinstance(metric_name, str) or not metric_name:
            raise RuntimeError(f"folly scenario {result_key} has no source_name")
        if metric_name != result_key:
            raise RuntimeError(
                f"folly scenario key {result_key} differs from source_name {metric_name}"
            )
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# folly {result['version']} 独立性能测试报告",
        "",
    ...
    lines.extend(
        [
            "",
            f"## 性能指标（官方场景逐字名称，共 {len(result.get('metrics', {}))} 个）",
            "",
            "| 指标 | 数值 | 单位 | 优化方向 |",
            "|---|---:|---|---|",
        ]
...
def finalize(
    ...
    benchmark = load_json(output_dir / "benchmark_folly.json")
    error = ""
    metrics: dict[str, Any] = {}
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_folly.json is missing or invalid")
