#!/usr/bin/env python3
"""Normalize protobuf's official Google Benchmark JSON into per-scenario metrics.

The official benchmark binary (benchmarks/benchmark.cc) is executed with the
parameters used by the official benchmarks/compare.py and emits Google
Benchmark JSON. This script extracts one median cpu_time value per official
scenario and preserves the official scenario name verbatim.
"""
...
def main() -> int:
    ...
    normalized = {
        "benchmark": "protobuf_official_benchmark",
        "software": "protobuf",
        "version": version,
        ...
        "parameters": {
            ...
            "benchmark_min_time_s": 0.05,
            "random_interleaving": True,
            "aggregation": "median",
            "official_entry": "benchmarks/benchmark.cc",
        },
        "metric_contract": {
    ...
    print(f"[protobuf-parse] normalized {len(results)} official scenarios")
    return 0
