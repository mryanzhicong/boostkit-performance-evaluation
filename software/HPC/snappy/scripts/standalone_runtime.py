#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for Snappy."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual Snappy version is empty")
...
def extract_metrics(
    ...
    if benchmark.get("software") != "snappy":
        raise RuntimeError("benchmark_snappy.json has an invalid software identity")
    if benchmark.get("version") != version or benchmark.get("architecture") != architecture:
        raise RuntimeError("benchmark_snappy.json identity differs from this run")
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# Snappy {result['version']} 独立性能测试报告",
        "",
...
def finalize(
    ...
    benchmark = load_json(output_dir / "benchmark_snappy.json")
    error = ""
    metrics: dict[str, Any] = {}
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_snappy.json is missing or invalid")
