#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for RapidJSON."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual RapidJSON version is empty")
...
def extract_metrics(benchmark: dict[str, Any]) -> dict[str, Any]:
    if benchmark.get("benchmark") != "rapidjson_perftest":
        raise RuntimeError("benchmark_perftest.json has an invalid benchmark name")
    results = benchmark.get("results")
    if not isinstance(results, dict):
        raise RuntimeError("benchmark_perftest.json is missing results")
    metrics: dict[str, Any] = {}
    for metric_name, result in results.items():
        if not isinstance(metric_name, str) or not metric_name:
            raise RuntimeError("benchmark_perftest.json contains an invalid metric name")
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# RapidJSON {result['version']} 独立性能测试报告",
        "",
...
def finalize(
    ...
    benchmark_path = output_dir / "benchmark_perftest.json"
    ...
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_perftest.json is missing or invalid")
