#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for OpenCV."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual OpenCV version is empty")
...
def extract_metrics(benchmark: dict[str, Any]) -> dict[str, Any]:
    if benchmark.get("benchmark") != "opencv_perf":
        raise RuntimeError("benchmark_opencv_perf.json has an invalid benchmark name")
    results = benchmark.get("results")
    if not isinstance(results, dict):
        raise RuntimeError("benchmark_opencv_perf.json is missing results")
    metrics: dict[str, Any] = {}
    for metric_name, result in results.items():
        if not isinstance(metric_name, str) or not metric_name:
            raise RuntimeError("benchmark_opencv_perf.json contains an invalid metric name")
        if not isinstance(result, dict):
            raise RuntimeError(f"metric {metric_name} is not an object")
        value = result.get("median_ms")
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise RuntimeError(f"metric {metric_name} is missing median_ms or it is not numeric")
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# OpenCV {result['version']} 独立性能测试报告",
        "",
    ...
    for group, metrics in metric_groups.items():
        ...
        for metric_name, metric in metrics:
            direction = "越小越好" if metric["direction"] == "lower_is_better" else metric["direction"]
            lines.append(
...
def finalize(
    ...
    benchmark_path = output_dir / "benchmark_opencv_perf.json"
    ...
    if command_status == "passed":
        try:
            if not benchmark:
                raise RuntimeError("benchmark_opencv_perf.json is missing or invalid")
