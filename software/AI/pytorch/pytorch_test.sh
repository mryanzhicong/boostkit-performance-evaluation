...
configure_runtime_paths() {
    ...
    if [[ -z "${PERF_WORK_DIR}" ]]; then
        PERF_WORK_DIR="/tmp/pytorch-perf/local-${PERF_RUN_ID}"
    ...
    export PERF_ACTUAL_VERSION_FILE TORCH_THREAD_LEVELS
}
...
cleanup_standalone_workdir() {
    ...
    if [[ "${PERF_WORK_DIR}" != /tmp/pytorch-perf/local-* || \
          "${PERF_WORK_DIR}" == "/tmp/pytorch-perf" ]]; then
