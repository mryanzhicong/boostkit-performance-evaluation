...
phase1_build() {
    ...
    if [ "${PERF_SKIP_SYSTEM_PACKAGES:-false}" = "true" ]; then
        log "PHASE1" "Using build dependencies preinstalled in the Runner image"
    else
      case "${os_id}" in
        ubuntu|debian)
...
testArchitectureMatchesExpected() {
    local arch expected
    arch="$(uname -m)"
    expected="${EXPECTED_ARCH:-${arch}}"
    [ "${arch}" = "amd64" ] && arch="x86_64"
    [ "${arch}" = "arm64" ] && arch="aarch64"
    [ "${expected}" = "amd64" ] && expected="x86_64"
    [ "${expected}" = "arm64" ] && expected="aarch64"
    assertEquals "Runner architecture must match EXPECTED_ARCH" "${expected}" "${arch}"
}
