#!/usr/bin/env python3
"""Provide self-contained environment collection and reporting for hnswlib."""
...
def record_build_info(
    ...
    if not actual_version:
        raise RuntimeError("actual hnswlib version is empty")
    atomic_write_json(
        output,
        {
...
def validate_identity(
    ...
    if payload.get("software") != "hnswlib":
        raise RuntimeError(f"{filename} has an invalid software identity")
    if payload.get("version") != version:
        raise RuntimeError(f"{filename} has an unexpected hnswlib version")
...
def render_report(result: dict[str, Any]) -> str:
    lines = [
        f"# hnswlib {result['version']} 独立性能测试报告",
        "",
